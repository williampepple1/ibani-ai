  ...
  return (
    <div className="glass-card">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
        <h2 className="text-lg sm:text-xl font-bold text-slate-900">Batch Translation</h2>
        {translations && (
          <button
            onClick={handleExport}
            className="flex items-center gap-1.5 px-3 py-2 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-all duration-300 min-h-[44px] sm:min-h-0"
          >
      ...
      <p className="text-slate-500 text-[11px] sm:text-xs font-semibold mb-4 sm:mb-6">
      ...
      <div className="mb-4 sm:mb-6 group">
        <div className="flex justify-between items-center mb-2 ml-1">
          <label className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest group-focus-within:text-primary-600 transition-colors duration-300">
        ...
        <div className="relative">
          <textarea
            ...
            className="input-premium min-h-[140px] sm:min-h-[160px] resize-none pb-10 font-mono text-[11px] sm:text-xs leading-relaxed"
            disabled={loading}
          />
          <div className="absolute bottom-3 right-3 sm:right-4 flex items-center gap-2">
            <span className="px-2 py-0.5 bg-white rounded text-[9px] sm:text-[10px] font-bold text-slate-400 border border-slate-100 shadow-sm uppercase tracking-wider">
              {textCount} Items
            </span>
          </div>
        </div>
        ...
        >
          {loading ? (
            <>
              ...
              <span className="tracking-widest uppercase text-[10px] sm:text-xs">Bulk Processing...</span>
            </>
          ) : (
            <>
              ...
              <span className="tracking-widest uppercase text-[10px] sm:text-xs">Translate All</span>
            </>
          )}
        </button>
        <button
      ...
      {translations && (
        <div className="animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="space-y-2 max-h-[300px] sm:max-h-[350px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
            {translations.translations.map((translation, index) => (
              <div
                key={index}
                className="group relative"
              >
                <div className="relative bg-white border border-slate-100 rounded-lg sm:rounded-xl p-3 shadow-sm transition-all duration-300">
                  <div className="flex items-start gap-2 sm:gap-3">
                    <div className="flex-shrink-0 w-5 h-5 sm:w-6 sm:h-6 premium-gradient text-white rounded text-center flex items-center justify-center font-bold text-[9px] sm:text-[10px] shadow-sm">
                      {index + 1}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-tight line-clamp-1">{translation.original_text}</p>
                      <p className="text-xs sm:text-sm text-slate-800 font-bold mt-0.5">{translation.translated_text}</p>
                    </div>
                  </div>
                </div>
              </div>
